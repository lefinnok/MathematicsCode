...
from sympy import symbols, Eq, solve, Rational, Symbol # Temporary Algebraic Solution
...
def multiInverse(matrix):#[tag: determinant, vector multiplication, identity matrix, scalar multiplication, adjoint matrix]
    ...
    if det(matrix) == 0:
        ...
        return scalarMulti((Rational(1,det(matrix))),adj(matrix))
...
def eigenValue(matrix):
    ...
    '''
    l = symbols('l')
    print(f'\ndet(matrix - scalarMulti({l}, IdentityMatrix(matrix.r)))')
    print(det(matrix - scalarMulti(l, IdentityMatrix(matrix.r))))
    return solve(det(matrix - scalarMulti(l, IdentityMatrix(matrix.r))),l)

def eigenVectors(matrix):
    sym = [Symbol(f'x{n+1}') for n in range(matrix.r)]
    symVec = T(Matrix([sym]))
    res = {}
    for eV in eigenValue(matrix):
        CharEqua = (matrix-IdentityMatrix(matrix.r)*eV)*symVec
        equations = [sum(row) for row in CharEqua.matrix]
        k = Symbol('k')
        equations.append(sym[-1]-k)
        res[eV] = solve(equations, *sym)
    return res
...
class Matrix():
    ...
    def __mul__(self,other):
        a = self
        b = other
        if type(other) == Matrix or issubclass(type(other), Matrix):
            if a.c != b.r:
                return 'Matricies invalid for vector multiplication.'
            return Matrix([[sum([valA * valB for valA,valB in zip(rowA,colB)]) for colB in T(b).matrix] for rowA in a.matrix])
        else:
            return Matrix([[val*Rational(other) for val in row] for row in self.matrix])
    ...
    def __pow__(self,other):
        ...
        if power == 1:
            ...
            return vectorMulti(matrix, matrix**(power-1))
...
print(A*B)
print(B*A)
#print(vectorMultiStr(C, D))
#print(C*D)
print(multiInverse(D))
'''
...
print(vectorMulti(a, scalarMulti(Rational(1,8),c)).form())
print(scalarMulti(Rational(1,8),c).form(),multiInverse(a).form())
'''
...
'''
'''
A = Matrix([[3,1,4],[-2,0,1],[1,2,2]])
B = Matrix([[1,0,2],[-3,1,1],[2,-4,1]])

C = Matrix([[1,2,0],[2,8,2],[0,2,1]])
